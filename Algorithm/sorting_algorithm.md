...
- 과정
    1. 각 숫자에 대해 최소 자릿수에 해당하는 숫자를 각각의 버킷에 넣어 정렬
    2. 버킷에서 숫자들을 빼와 배열에 삽입 (각 버킷은 큐이므로 선입선출에 따름)
    3. 1, 2를 최대 자릿수까지 반복

<img src = "https://blog.kakaocdn.net/dn/lOExy/btqyY24KHRA/cj3LlA2vMGenAps8Xazyz0/img.png" width = "400px">

- 데이터 개수를 $n$, 최대 자릿수를 $d$라고 할 때 시간복잡도는 $O(dn)$임 (꽤 빠른 편)
- 버킷 구성을 위한 추가적 메모리 필요
- 정렬 가능한 데이터 타입이 한정적
...
- 데이터의 개수를 세 정렬
- 과정
    - 데이터 범위를 인덱스로 갖는 배열 생성
    - 정렬하려는 배열을 순회하며 데이터에 해당하는 계수 배열의 인덱스 값을 1씩 증가
    - 정렬하려는 배열의 순회가 끝나면 계수 배열을 순회하며 값을 정렬

<img src = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQP40b7CzsDGmMjaTVg-Y_yvYry4Zj8ou3spw&usqp=CAU" width = "400px">

- 정렬하려는 데이터의 개수가 $n$개, 데이터의 최댓값이 $k$일 때 시간 복잡도는 $O(n + k)$가 된다
- 제약조건 : 데이터의 범위가 0 이상인 정수여야 함
- 데이터의 최댓값이 무한대에 수렴할 경우 시간복잡도 역시 무한에 수렴 -> 데이터가 시간 복잡도에 영향
- 데이터 범위만큼의 배열 생성이 필요 -> 메모리 공간이 추가로 필요
